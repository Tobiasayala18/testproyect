...
@app.get("/resumen_porcentajes")
def resumen_porcentajes():
    df_comp = leer_cache(TABLA_COMPETENCIA_CACHE)
    df_nuest = leer_cache(TABLA_NUESTRO_CACHE)
    ...
    for idx, row in df_nuest.iterrows():
        ...
        # Detectar precio propio
        try:
            precio_nuestro = float(row.get("precio", None) or 0)
        except:
            continue
        ...
        try:
            precio_comp = float(comp_row.get("precio", None) or 0)
        except:
            continue

        if precio_nuestro == 0 or precio_comp == 0:
            continue

        diferencia = ((precio_nuestro - precio_comp) / precio_comp) * 100

        detalles.append({
            "producto": row.to_dict(),
            "competencia": comp_row.to_dict(),
            "diferencia_porcentual": diferencia
        })

    if detalles:
        promedio = sum([d["diferencia_porcentual"] for d in detalles]) / len(detalles)
    else:
        promedio = 0
